...
SELECT NOME AS PIOR_VENDEDOR, MARCO AS VALOR FROM VENDEDORES
WHERE MARCO = (SELECT MIN(MARCO) FROM VENDEDORES); /* A INNER QUERIE OU SUBQUERIE É RESOLVIDA PRIMEIRO! */

SELECT NOME AS MELHOR_VENDEDOR, MARCO AS VALOR FROM VENDEDORES
WHERE MARCO = (SELECT MAX(MARCO) FROM VENDEDORES);

SELECT AVG(FEVEREIRO) FROM VENDEDORES;
SELECT NOME AS PRINCIPAIS_VENDEDORES, FEVEREIRO AS VALOR FROM VENDEDORES
WHERE FEVEREIRO > (SELECT AVG(FEVEREIRO) FROM VENDEDORES);


SELECT JANEIRO, FEVEREIRO, MARCO, (JANEIRO + FEVEREIRO + MARCO) AS 'TOTAL', TRUNCATE(((JANEIRO + FEVEREIRO + MARCO)/3),2) FROM VENDEDORES;
